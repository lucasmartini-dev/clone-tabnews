...
describe("GET /api/v1/user", () => {
  describe("Anonymous user", () => {
    test(`Retrieving the endpoint`, async () => {
      ...
      expect(responseBody).toEqual({
        ...
        action: `Please verify if your user has the "read:session" feature.`,
        status_code: 403,
      });
    });
  });
  ...
  describe("Default user", () => {
    ...
    test("With nonexistent session", async () => {
      const nonexistentToken = "n0n-3x1st3ntT0k3n4ndS3ss10n_curso.dev";
    ...
    test("With one minute left before expire valid session", async () => {
      const oneMinuteLeftBeforeExpire =
        session.EXPIRATION_IN_MILLISECONDS - 60 * 1000;
      const backToDate = new Date(Date.now() - oneMinuteLeftBeforeExpire);
      jest.useFakeTimers({ now: backToDate });
